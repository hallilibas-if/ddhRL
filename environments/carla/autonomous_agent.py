...
class Agent(gym.Env):
    ...
    def __init__(self, config, _id):
        ...
        self.channel = self.observation_space.shape[2]
        self._id = _id
        ...
        self.oldKeepLane = 0
        self.obs_im = np.zeros((self.width, self.height, self.channel))
        self.obs_Scalar = np.zeros((self.width, self.height))
        self.cycleSpeedCurrent = deque([0,0,0,0], maxlen=4) 
        self.cycleSpeedLimit = deque([0,0,0,0], maxlen=4) 
        self.cycleSpeedTarget = deque([0,0,0,0], maxlen=4) 
        self.cycleAcc = deque([0,0,0,0], maxlen=4) 
    ...
    def reset(self):
        ...
        self.obs_im = np.zeros((self.width, self.height, self.channel))  #ToDo Shawan: Change here to a more dynamic approach using spaces.Box
        self.obs_Scalar = np.zeros((self.width, self.height))
        return self.obs_im
    ...
    def _get_observation_conductor(self,tSpeed, actions):
        ...
        self.cycleSpeedCurrent.appendleft(round((self.current_speed-4)/8 ,1)) 
        self.cycleSpeedLimit.appendleft(round((self.speedLimit-4)/8 ,1))  
        self.cycleAcc.appendleft(round(self.acceleration,1))        

        self.obs_Scalar[0]=self.cycleSpeedCurrent
        self.obs_Scalar[1]=self.cycleSpeedLimit
        self.obs_Scalar[2]=self.cycleAcc

        obs = np.concatenate((self.obs_im ,np.expand_dims(np.array(self.obs_Scalar), axis=-1)), axis=-1)
    ...
    def _get_observation_controller(self):
        ...
        """      
        self.cycleSpeedTarget.appendleft(round((self.tagetSpeed_conductor-4)/8 ,1))
        self.obs_Scalar[1]=self.cycleSpeedTarget
        obs = np.concatenate((self.obs_im ,np.expand_dims(np.array(self.obs_Scalar), axis=-1)), axis=-1)
    ...
    def _calculate_reward_conductor(self, rewards):
        ...
        self.reward = 1 -(self.c_collision * self.reward_collision) - (self.c_speed * self.reward_difTargetSpeed)
