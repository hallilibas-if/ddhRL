...
def _tf_build_networks(configuration):
    ...
    actor_critic_feature_map_input = tf.keras.layers.Input(shape=[actor_critic_input_shape[-3],
																  ...
																  actor_critic_input_shape[-1]+192]) # actor_critic_input_shape[4]+10]) 2*seq_length! Because after dpc there is no seq_length! Everything is only a big qubic!ToDo Shawan: Changed here to a dynamic way of implementation!
    ...
    road_speed_map_input = tf.keras.layers.Input(shape=[configuration['NUM_DPC_FRAMES']])
    agent_speed_map_input = tf.keras.layers.Input(shape=[configuration['NUM_DPC_FRAMES']])
    agent_action_map_input = tf.keras.layers.Input(shape=[configuration['NUM_DPC_FRAMES']])
