...
def _tf_build_networks(configuration):
    ...
    actor_critic_input_shape = (1,1, 4, 4, 2048) # zuvor  für DPC(1, 1, 2, 2, 2048), wenn MoCo mit 128x128 dann (1, 1, 4, 4, 432)  
    ...
    actor_critic_feature_map_input = tf.keras.layers.Input(shape=[actor_critic_input_shape[-3],
																  ...
																  actor_critic_input_shape[-1]+54]) # actor_critic_input_shape[4]+10]) 2*seq_length! Because after dpc there is no seq_length! Everything is only a big qubic!ToDo Shawan: Changed here to a dynamic way of implementation!
    ...
    road_speed_map_input = tf.keras.layers.Input(shape=[4,1])
    agent_speed_map_input = tf.keras.layers.Input(shape=[4,1])
    agent_action_map_input = tf.keras.layers.Input(shape=[4,1])
    ...
    return actor_critic_shared, actor, critic
...
def _tf_build_actor_net(x, configuration, agent_speed, road_speed, agent_actions,actor_cnn_gru_states, actor_fc_gru_states):
    ...
    agent_speed = tf.keras.layers.Flatten()(agent_speed)
    road_speed = tf.keras.layers.Flatten()(road_speed)
    agent_actions = tf.keras.layers.Flatten()(agent_actions)

    agent_speed = tf.keras.layers.Dense(units=4, kernel_initializer="he_normal", kernel_regularizer="l1_l2", activation='relu')(agent_speed)
    road_speed = tf.keras.layers.Dense(units=4, kernel_initializer="he_normal", kernel_regularizer="l1_l2", activation='relu')(road_speed)
    agent_actions = tf.keras.layers.Dense(units=4, kernel_initializer="he_normal", kernel_regularizer="l1_l2", activation='relu')(agent_actions)
