...
class CustomTFModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.encoder, self.actor_critic_shared_model, self.actor_model, self.critic_model = build_networks(self.configuration)
        
        self.input_name = self.encoder.get_inputs()[0].name

        self.save_dummy = tf.keras.Model([self.encoder, self.actor_model, self.critic_model])
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        #Get previous obs
        stacked_observations = input_dict["prev_n_obs"][:,:-1,:,:,0:3] 
        ...
        all_observations = np.concatenate((current_observation, stacked_observations),axis =1)
     
        inputs =  np.array(all_observations, dtype=np.float32)  
        inputs = inputs.transpose(0, 4, 1, 2,3)
        print("Shape of stacked obs inside inputs: ", inputs.shape) #(32, 3, 1, 128, 128)
        #inputs = tf.cast(inputs, tf.float32)

        stacked_scalar_prev = stacked_observations[:,:,:,:,-1] 
        ...
        input_raw = np.array(inputs[None,:,:,:,:,:])
        feature_map = self.encoder.run(None, {self.input_name: input_raw})[-1]
