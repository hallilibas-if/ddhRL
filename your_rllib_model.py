...
class YourModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        super(YourModel, self).__init__(obs_space, action_space, num_outputs, model_config, name)

        # For logging actions mean an variance
        self.configuration = model_config

        build_networks = _tf_build_networks(self.configuration)
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # Dummy input with shape (1, 8, 8, 432)
        feature_map = np.zeros((1, 8,8, 432))
